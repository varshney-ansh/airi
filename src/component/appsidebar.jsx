        ...
        <div className="
            ...
            <div className="w-full max-w-[256px] px-1 pb-0.5 pt-4 select-none">
                <div className="flex justify-between items-center group-data-[state=close]:flex-col group-data-[state=close]:gap-2 group-data-[state=close]:items-start">
                    ...
                    <div onClick={() => { const sidebar = document.getElementById("sidebar"); const newState = sidebar.getAttribute('data-state') === 'open' ? 'close' : 'open'; sidebar.setAttribute('data-state', newState); }} className="text-sm outline-offset-1 flex items-center justify-center hover:bg-[#ffffff14] transition-all duration-100 cursor-pointer bg-transparent text-[#c2cadf] font-semibold w-9 h-9 rounded-lg">
                    ...
                    {session && (
                        <div className="relative">
                            <button
                                ...
                                style={{ anchorName: "--profile-anchor" }} // Anchor Name
                                className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-1.5 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out"
                            >
                                <img className="size-6 rounded-full" src={`data://image/png;base64,${profilePicBase64 || session.user.picture}`} alt="profile-dp" />
                                <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-semibold items-center gap-1.5 text-start text-[15px] truncate text-[#e5ebfa]">
                                    {session.user.name}
                                </span>
                            </button>
                            <div
                                popover="auto"
                                id="profilemenu"
                                style={{ positionAnchor: "--profile-anchor" }} // Connect to anchor
                                className="popover-content p-1 bg-[#151a28] border border-[#ffffff14] rounded-xl shadow-2xl text-[#e5ebfa] min-w-69"
                            >
                                <button className="font-semibold cursor-pointer flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[#ffffff14] transition-all duration-100">
                                    Settings
                                </button>
                                <button onClick={() => window.location.href = "/auth/logout"} className="font-semibold cursor-pointer flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[#ffffff14] transition-all duration-100">
                                    Sign out
                                </button>
                            </div>
                        </div>
