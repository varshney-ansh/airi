...
const AppSideBar = ({ session, profilePicBase64 }) => {
  const [isShowingSettingModal, setIsShowingSettingModal] = useState(false);
  return (
    <>
      {isShowingSettingModal && (
        <SettingsModal hideModal={() => setIsShowingSettingModal(false)} />
      )}
      <div
        className="
      ...
      >
        <div className="w-full max-w-[256px] px-1 pb-0.5 pt-4 select-none">
          <div className="flex justify-between items-center group-data-[state=close]:flex-col group-data-[state=close]:gap-2 group-data-[state=close]:items-start">
            <h1 className="font-semibold text-[#e5ebfa] text-[18px] outline-offset-1 pl-2.5">
              Airi
            </h1>
            <div
              onClick={() => {
              ...
              className="text-sm outline-offset-1 flex items-center justify-center hover:bg-[#ffffff14] transition-all duration-100 cursor-pointer bg-transparent text-[#c2cadf] font-semibold w-9 h-9 rounded-lg"
            >
              <svg
                viewBox="0 0 24 24"
                fill="#e5ebfa"
          ...
          <div className="flex flex-col gap-1 pt-1 items-center group-data-[state=close]:items-start">
            <button className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-2 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out">
              ...
              <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-semibold items-center gap-1.5 text-start text-[16px] text-[#e5ebfa]">
                New chat
              </span>
            </button>
            <button className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-2 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out">
          ...
          <div className="h-6 rounded-xl py-3 w-full px-2 group-data-[state=close]:hidden">
            <div className="h-0 border-t border-black/10 dark:border-white/20"></div>
          </div>
          {/* chats */}
          <div className="group-data-[state=close]:hidden">
            {!session && (
              <p className="px-2.5 pb-6 font-semibold text-[#c2cadf] text-sm">
            ...
            {session && (
              <ChatItem
                chat={{ id: 1, title: "Introduction to Machine Learning" }}
              />
            )}
          </div>
          {/* account */}
              ...
              <div className="relative">
                ...
                >
                  <img
                    ...
                    src={`data://image/png;base64,${profilePicBase64 || session.user.picture}`}
                    alt="profile-dp"
                  />
                ...
                >
                  <button
                    onClick={() => setIsShowingSettingModal(true)}
                    className="font-semibold cursor-pointer flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[#ffffff14] transition-all duration-100"
                  >
                    Settings
                  </button>
                  <button
