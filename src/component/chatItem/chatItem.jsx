export default function ChatItem({ chat }) {
    const popoverId = `chat-options-${chat.id}`;
    const anchorName = `--anchor-${chat.id}`;

    return (
        <div className="flex size-full items-center justify-between pb-1.5 rounded-2xl hover:bg-bg-hover px-0.5 py-0.5 cursor-pointer transition-all duration-100 group/item">
            <div className="flex h-full min-w-0 flex-1 items-center justify-between gap-1.5 px-2.5 font-medium text-text-muted text-[14px]">
                <p className="truncate text-text-muted" title={chat.title}>
                    {chat.title}
                </p>
            </div>
            ...
            >
                <button
                    onClick={() => console.log('Rename', chat.id)}
                ...
                <button
                    onClick={() => console.log('Share', chat.id)}
                    className="cursor-pointer flex w-full font-medium items-center gap-1 rounded-lg px-3 py-2 text-[14px] text-text-primary hover:bg-bg-hover transition-all"
                >
