...
const ChatMain = ({ userId, chatId = "test", user_name }) => {
  ...
  const handleOnSubmit = useCallback(
    (inputData) => {
      ...
      if (!content && (!inputData.files || inputData.files.length === 0))
        return;
      ...
      <div className="bg-[#151a28] h-[98vh] rounded-md border border-[#ffffff14] flex flex-col relative">
        ...
        <div
          onClick={() => {
            ...
            const newState =
              sidebar.getAttribute("data-state") === "open" ? "close" : "open";
            sidebar.setAttribute("data-state", newState);
          }}
          className="absolute top-2.5 left-2.5 md:hidden z-30 text-sm outline-offset-1 flex items-center justify-center hover:bg-[#ffffff14] transition-all duration-100 cursor-pointer bg-transparent text-[#c2cadf] font-semibold w-9 h-9 rounded-lg"
        >
          <svg
            viewBox="0 0 24 24"
            fill="#e5ebfa"
        ...
        <div
          data-showgreet={messages.length === 0}
          className="group data-[showgreet=true]:flex-[0.5] flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[#ffffff14] scrollbar-track-transparent"
        >
